...
describe('inspect: why does natural BJ not auto-advance to dealer', () => {
  it('natural p1 BJ + p2 stand should advance to dealer', () => {
    for (let trial = 0; trial < 200; trial++) {
      ...
      if (naturalP1) {
        console.log(`Trial ${trial}: P1 natural BJ`);
        // P1's hand is settled (stood=true, blackjack=true in dealing phase)
        // P1's all_settled flag:
        console.log('P1 all_settled:', privP1.players?.p1?.all_settled);
        console.log('P2 all_settled:', module.getPrivateState(roomId, 'p2') as any)
        
        // p1 stand rejected (already settled), so we only need p2 to stand
        const s2 = module.handleInput(roomId, 'p2', 'action', { action: 'stand' });
        console.log('P2 stand accepted:', s2.accepted, 'Phase after:', module.getPhaseState(roomId).phaseType);
        
        // Does p1's natural have all_settled already submitted to input_gate?
        // The key question: did dealing fire a declarative confirm for p1?
        break;
      }
    }
  });
