...
class VillageModule implements GameModule {
  ...
  setup(players: Player[], ctx: GameContext): void {
    ...
    const state: VillageGameState = {
      ...
      dayNumber: 0,
      currentPhase: VOSPhase.ROLE_REVEAL,
      alive,
      nightActions: [],
  ...
  getPhaseState(roomId: string): PhaseState {
    ...
    switch (state.currentPhase) {
      case VOSPhase.ROLE_REVEAL: timerTotalMs = VOS_ROLE_REVEAL_TIME_SECONDS * 1000; break;
      case VOSPhase.NIGHT: timerTotalMs = VOS_NIGHT_TIME_SECONDS * 1000; break;
      case VOSPhase.NIGHT_RESULT: timerTotalMs = VOS_NIGHT_RESULT_TIME_SECONDS * 1000; break;
      case VOSPhase.DAY: timerTotalMs = VOS_DAY_TIME_SECONDS * 1000; break;
      case VOSPhase.VOTE: timerTotalMs = VOS_VOTE_TIME_SECONDS * 1000; break;
      case VOSPhase.VOTE_RESULT: timerTotalMs = VOS_VOTE_RESULT_TIME_SECONDS * 1000; break;
    }
    return {
  ...
  getPublicState(roomId: string): Record<string, unknown> {
    ...
    const publicState: VillagePublicState = {
      ...
      votes: state.currentPhase === VOSPhase.VOTE || state.currentPhase === VOSPhase.VOTE_RESULT
  ...
  getPrivateState(roomId: string, playerId: string): Record<string, unknown> {
    ...
    if (state.currentPhase === VOSPhase.NIGHT && isAlive && !state.nightActedPlayerIds.has(playerId)) {
    ...
    if (state.currentPhase === VOSPhase.VOTE && isAlive && !state.dayVotes.has(playerId)) {
  ...
  private handleNightAction(
    ...
    if (state.currentPhase !== VOSPhase.NIGHT) {
  ...
  private handleDayVote(
    ...
    if (state.currentPhase !== VOSPhase.VOTE) {
  ...
  private startNight(roomId: string): void {
    ...
    state.dayNumber++;
    state.currentPhase = VOSPhase.NIGHT;
    state.nightActions = [];
    ...
    state.ctx.startTimer(
      ...
      () => {
        const s = this.states.get(roomId);
        if (!s || s.currentPhase !== VOSPhase.NIGHT) return;
  ...
  private resolveNightPhase(roomId: string): void {
    const state = this.states.get(roomId);
    if (!state) return;
    if (state.currentPhase !== VOSPhase.NIGHT) return;

    state.ctx.stopTimer();
    state.currentPhase = VOSPhase.NIGHT_RESULT;
    ...
    if (winTeam) {
      ...
      state.ctx.startTimer(
        ...
        () => {
          const s = this.states.get(roomId);
          if (!s || s.currentPhase !== VOSPhase.NIGHT_RESULT) return;
  ...
  private startDay(roomId: string): void {
    ...
    state.ctx.stopTimer();
    state.currentPhase = VOSPhase.DAY;
    ...
    state.ctx.startTimer(
      ...
      () => {
        const s = this.states.get(roomId);
        if (!s || s.currentPhase !== VOSPhase.DAY) return;
  ...
  private startVote(roomId: string): void {
    ...
    state.ctx.stopTimer();
    state.currentPhase = VOSPhase.VOTE;
    state.dayVotes = new Map();
    ...
    state.ctx.startTimer(
      ...
      () => {
        const s = this.states.get(roomId);
        if (!s || s.currentPhase !== VOSPhase.VOTE) return;
  ...
  private resolveVote(roomId: string): void {
    const state = this.states.get(roomId);
    if (!state) return;
    if (state.currentPhase !== VOSPhase.VOTE) return;

    state.ctx.stopTimer();
    state.currentPhase = VOSPhase.VOTE_RESULT;
    ...
    if (winTeam) {
      ...
      state.ctx.startTimer(
        ...
        () => {
          const s = this.states.get(roomId);
          if (!s || s.currentPhase !== VOSPhase.VOTE_RESULT) return;
          this.startNight(roomId);
        },
      );
    }
  ...
  private buildVoteTally(state: VillageGameState) {
    ...
    return [...tally.entries()].map(([targetId, voterNames]) => {
      ...
      return {
        ...
        voterNames: state.currentPhase === VOSPhase.VOTE_RESULT ? voterNames : [],
      };
    });
  }
}
